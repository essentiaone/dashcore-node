...
BlockController.prototype.checkBlockHash = function(req, res, next) {
  ...
  if (hash.length < 64 || !isHexadecimal(hash)) {
    this.node.services.essd.getBlockHeader(parseInt(hash), function(err, info) {
      if (err) {
...
BlockController.prototype.blockHeader = function(req, res, next) {
	...
	self.node.services.essd.getBlockHeader(hash, function(err, info) {
		if (err) {
			return self.common.handleErrors(err, res);
		}
...
BlockController.prototype.blockHeaders = function(req, res, next) {
	...
	if(blockIdentifier.length===64){
		...
		var result = self.node.services.essd.getBlockHeaders(hash, cb, nbOfBlockToFetch);
	}else{
		var height = blockIdentifier;
		var result = self.node.services.essd.getBlockHeaders(height, cb, nbOfBlockToFetch);
	}
...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached) {
    blockCached.confirmations = self.node.services.essd.height - blockCached.height + 1;
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  this.node.services.essd.getBlockHeader(parseInt(height), function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.essd.getRawBlock(hash, function(err, blockBuffer) {
      if (err) {
        return next(err);
      }
      ...
      // would require changes in essx-node.
      ...
      self.node.services.essd.getBlockHeader(hash, function(err, blockHeader) {
        if (err) {
          return next(err);
        }
        ...
        var confirmations = self.node.services.essd.height - height + 1;
...
BlockController.prototype._getBlockSummary2 = function(hash, next) {
  var self = this;
  // console.log('hash', hash)
  function finish(result) {
    return next(null, result);
  }
  console.log('self.blockSummaryCache', self.blockSummaryCache.get(hash))
  var summaryCache = self.blockSummaryCache.get(hash);
...
//     console.log('lte, gte', lte, gte)
//   self.node.services.essd.getBlockHashesByTimestamp(lte, gte, function(err, hashes) {
//     if(err) {
...
//       function(err, blocks) {
//         if(err) {
//           console.log("eeeerrrrrrrrror here", err)
//           return self.common.handleErrors(err, res);
//         }
//
...
BlockController.prototype.list = function(req, res) {
  ...
  //pagination
  var limit = parseInt(req.query.limit || BLOCK_LIMIT);
  var blochHeight = parseInt(req.query.from || null);
  console.log('blochHeight', blochHeight)
  var first = blochHeight ? false : true
  self.node.services.essd.getBlockHashesByLimit(limit, blochHeight, function(err, hashes) {
    if(err) {
      console.log('error here')
      return self.common.handleErrors(err, res);
    }
    ...
    async.mapSeries(
        hashes,
        function(hash, next) {
        self._getBlockSummary2(hash, next);
      },
        function(err, blocks) {
          if(err) {
            console.log("eeeerrrrrrrrror here", err)
          ...
          var data = {
            ...
            length: blocks.length,
            pagination: {
              next: first ? blocks[0].height : blochHeight + limit,
              prev: first ? blocks[0].height - limit : blochHeight - limit,
              first: first
            }
          };
          res.jsonp(data);
...
BlockController.prototype.getPreviousBlock = function(prevHash, cb) {
  ...
  if(prevHash===null) cb(null,null);
      ...
      self.node.getBlock(prevHash, function (err, block) {
          ...
          self.node.services.essd.getBlockHeader(prevHash, function (err, info) {
              if (err) {
