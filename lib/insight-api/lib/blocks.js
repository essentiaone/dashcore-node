...
BlockController.prototype.list = function(req, res) {
  ...
  //pagination
  var limit = parseInt(req.query.limit || BLOCK_LIMIT);
  var blockHeight = parseInt(req.query.from || null);
  var first = blockHeight ? false : true
  self.node.services.essd.getBlockHashesByLimit(limit, blockHeight, function(err, hashes) {
    if(err) {
      console.log('error here')
      return self.common.handleErrors(err, res);
    }
    ...
    async.mapSeries(
        ...
        function(err, blocks) {
          ...
          var data = {
            ...
            length: blocks.length,
            pagination: {
              next: first ? blocks[0].height : blockHeight + limit,
              prev: first ? blocks[0].height - limit : blockHeight - limit,
              first: first
            }
          };
          res.jsonp(data);
