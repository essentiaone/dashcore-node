...
StatusController.prototype.getInfo = function(callback) {
  this.node.services.essd.getInfo(function(err, result) {
    if (err) {
      return callback(err);
    }
    var info = {
...
StatusController.prototype.getLastBlockHash = function(callback) {
  this.node.services.essd.getBestBlockHash(function(err, hash) {
    if (err) {
      return  callback(err);
    }
    callback(null, {
...
StatusController.prototype.getDifficulty = function(callback) {
  this.node.services.essd.getInfo(function(err, info) {
    if (err) {
      return callback(err);
    }
    callback(null, {
...
StatusController.prototype.sync = function(req, res) {
  ...
  this.node.services.essd.isSynced(function(err, synced) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    if (synced) {
    ...
    self.node.services.essd.syncPercentage(function(err, percentage) {
      if (err) {
        return self.common.handleErrors(err, res);
      }
      var info = {
        status: status,
        blockChainHeight: self.node.services.essd.height,
        syncPercentage: Math.round(percentage),
        height: self.node.services.essd.height,
        error: null,
        type: 'bitcore node'
      };
...
// Mempool information
StatusController.prototype.mempool = function(req, res) {
  var self = this;

  self.node.services.essd.getMemPoolInfo(function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp(info);
