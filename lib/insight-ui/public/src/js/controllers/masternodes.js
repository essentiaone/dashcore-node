...
angular.module('insight.masternodes').controller('MasternodesController',
  ...
  $scope.getRewardFrequency = function(tiers) {
      ...
      /* for (var tier in tiers) {
          if (tier === 'MARS' && $scope.activeTab.toUpperCase() === 'MARS') {
              allocated += 300000 * tiers[tier].count;
          } else if (tier === 'MERCURY') {
              allocated += 100000 * tiers[tier].count;
          }
      } */
      var activeTab = $scope.activeTab.toUpperCase();
      if (activeTab === 'MARS') {
        allocated += 300000 * tiers[activeTab].count;
      } else if (activeTab === 'MERCURY') {
        allocated += 100000 * tiers[activeTab].count;
      }
