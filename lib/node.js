...
Node.prototype._startService = function(serviceInfo, callback) {
  ...
  log.info('Starting ' + serviceInfo.name); // TODO: update service name in essx-node.json
  ...
  async.eachSeries(
    ...
    function(service, next) {
      if (self.services[service.name]) {
        log.info('Stopping ' + service.name); // TODO: update service name in essx-node.json
        self.services[service.name].stop(next);
      } else {
        log.info('Stopping ' + service.name + ' (not started)'); // TODO: update service name in essx-node.json
        setImmediate(next);
      }
    },
    callback
