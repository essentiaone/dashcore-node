...
/**
 * Will return the path and default essx-node configuration. It will search for the
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.ess');
  var defaultConfigFile = path.resolve(defaultPath, './essx-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['essd', 'web'];
