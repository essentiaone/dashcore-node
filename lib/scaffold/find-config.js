...
/**
 * Will return the path and essx-node configuration
...
function findConfig(cwd) {
  ...
  while (!fs.existsSync(path.resolve(directory, 'essx-node.json'))) {
  ...
  return {
    path: directory,
    config: require(path.resolve(directory, 'essx-node.json'))
  };
}
