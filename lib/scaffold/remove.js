...
/**
 * Will remove a service from essx-node.json
...
/**
 * Will remove the Node.js service and from the essx-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The essx-node configuration directory
...
function remove(options, done) {
  ...
  var dashcoreConfigPath = path.resolve(configPath, 'essx-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(dashcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a essx-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to essx-node.json
