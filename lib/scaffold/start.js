...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both dash.conf and essx-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/dashevo/essx-node/blob/dashd/docs/upgrade.md \n');
        ...
        servicesConfig: {
          essd: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/.ess')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed essx-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './essx-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in essx-node services directory
