...
Dash.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['getBlockHashesByLimit', this, this.getBlockHashesByLimit, 2],
    ...
    ['getMemPoolInfo', this, this.getMemPoolInfo, 0]
  ];
  return methods;
};
...
Dash.prototype.subscribe = function(name, emitter) {
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'essd/' + name, 'total:', this.subscriptions[name].length);
};
...
Dash.prototype.unsubscribe = function(name, emitter) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'essd/' + name, 'total:', this.subscriptions[name].length);
};
...
Dash.prototype.subscribeAddress = function(emitter, addresses) {
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'essd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
...
Dash.prototype.unsubscribeAddress = function(emitter, addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'essd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
...
Dash.prototype._getConfig = function(configObj) {
  var config = '';
...
Dash.prototype._loadSpawnConfiguration = function(node) {
  ...
  var configPath = path.resolve(spawnOptions.datadir, './ess.conf');

  log.info('Using ESS config file:', configPath);

  this.spawn = {};
  this.spawn.agent = this.options.spawn.agent;
  this.spawn.datadir = this.options.spawn.datadir;
  this.spawn.zmqPort = this.options.spawn.zmqPort;
  this.spawn.nodePort = this.options.spawn.nodePort;
  ...
  _.extend(this.spawn.config, this._getConfFromJSON());
  if (fs.existsSync(configPath)) {
    _.extend(this.spawn.config, this._parseDashConf(configPath));
  }

  if (!fs.existsSync(configPath)) {
    var defaultConfig = this._getConfig(this.spawn.config);
...
Dash.prototype._getConfFromJSON = function() {
  return this.options.parameters ? this.options.parameters : {};
}

Dash.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of essx-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of essx-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of essx-node. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that essd is undergoing a reindex. ' +
             ...
             'of essx-node services will start.');
    node._reindex = true;
  }
};
...
Dash.prototype._getDefaultConf = function() {
  ...
  if (this.node.network === dashcore.Networks.testnet) {
    networkOptions.rpcport = RPC_PORT_TESTNET;
  }
...
Dash.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === dashcore.Networks.testnet) {
    networkPath = 'testnet3/ess.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/ess.conf';
    }
  }
  return networkPath;
};
...
Dash.prototype._zmqBlockHandler = function(node, message) {
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('essd/hashblock', message.toString('hex'));
    }
  }
  ...
  if (hex !== self.tiphash) {
    ...
    node.client.getBlock(self.tiphash, function(err, response) {
      if (err) {
        ...
        log.info('_updateTip', response.result.height)
        self.height = response.result.height;
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('essd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Dash.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('essd/rawtransaction', message.toString('hex'));
    }
...
Dash.prototype._zmqTransactionLockHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('essd/transactionlock', message.toString('hex'));
    }
...
Dash.prototype._stopSpawnedDash = function(callback) {
  var self = this;
  self.stopProcess = spawn('docker',
      ['rm',
        '-f',
        `$(docker ps -q -f name=essd)`],
      { stdio: 'inherit',
        detached: true,
        shell: true }
  );

  self.stopProcess.on('error', function(err) {
    return callback(err);
  });

  self.stopProcess.once('exit', function(code) {
    log.info('KILL essd process with code:', code);
...
Dash.prototype._spawnChildProcess = function(callback) {
  ...
  try {
    ...
    log.info('catch error here')
    return callback(e);
  }
  self._stopSpawnedDash(function(err, code) {
    if (err) {
      return callback(err);
    }

    log.info('Stop ess process with code: ', code);
    log.info('Starting ess process', self.spawn.agent);
    self.spawn.process = spawn('docker', ['run', '-i',
      '-p', self.spawn.zmqPort + ':' + self.spawn.zmqPort,
      '-p', self.spawn.nodePort + ':' + self.spawn.nodePort,
      '-p', self.spawn.config.rpcport + ':' + self.spawn.config.rpcport,
      '-e', 'RPCUSER=' +self.spawn.config.rpcuser,
      '-e', 'RPCPASS=' + self.spawn.config.rpcpassword,
      '--name=essd',
      '-v', self.spawn.datadir + ':/root/.ess',
      self.spawn.agent], {
        detached: true,
        stdio: 'inherit',
        shell: true
    ...
    self.spawn.process.on('error', function(err) {
      console.log('self.spawn.process.on error', err)
      self.emit('error', err);
    });
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('ESS process unexpectedly exited with code:', code);
        log.warn('Restarting ess child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Ess process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      node.client = new DashdRPC({
        protocol: 'http',
        host: self.spawn.config.host || '127.0.0.1',
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn essd.'));
      }
...
Dash.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    self._initZmqSubSocket(node, self.options.parameters.zmqpubrawtx);
    self._subscribeZmqEvents(node);
...
Dash.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  ...
  /* Issue: it's a temporary solution to remove duplicates and spent addresses */
  // if (queryMempool) {
        ...
        self.getAddressTxids(addresses, {queryMempool: false}, function(err, txids) {
          ...
          self.getAddressBalance(addresses, options, function (err, data) {
            ...
            self.client.getAddressMempool({'addresses': addresses}, function (err, response) {
              ...
              var mp = response.result;
              mempoolTxids = self._getTxidsFromMempool(mp);
              if(mempoolTxids.length === summary.appearances) {
                summary.unconfirmedAppearances = 0;
                summary.unconfirmedBalance = 0;
                mempoolTxids = [];
                return done();
              } else if (mempoolTxids.length > summary.appearances) {
                var newTx = mempoolTxids.filter(tx => {txids.indexOf(tx) + 1 });
                summary.unconfirmedAppearances = newTx.length;
                summary.unconfirmedBalance = mp.satoshis;
                mempoolTxids = newTx;
                return done();
              }
              mempoolTxids = self._getTxidsFromMempool(response.result);
...
Dash.prototype.getMemPoolInfo = function(callback) {
  var self = this;
    self._tryAllClients(function(client, done) {
      client.getMemPoolInfo(function(err, response) {
        if (err) {
          return done(self._wrapRPCError(err));
        }
        done(null, response.result);
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, true, function(err, response) {
          ...
          self.blockCache.set(blockhash, response.result);
          done(null, response.result);
        });
      }, callback);
    }
...
Dash.prototype._getBlockHashes = function(height, limit, callback) {
  var self = this;
  var arr = []
  for(var i = height; i > height - limit; i--) {
    arr.push(i);
  }
    async.map(arr, function(height, callback) {
      self.client.getblockhash(height, function(err, hash) {
        if (err) {
          throw new Error(err);
        }
...
/**
 * Will retrieve an array of block hashes within a range of count
 * @param {Number} count - How mutch blocks needed
 * @param {Number} low - The older timestamp in seconds
 * @param {Function} callback
 */
Dash.prototype.getBlockHashesByLimit = async function(limit, height, callback) {
  var self = this;
  if (!height) {
   self.client.getblockcount(function(err, response) {
        if (err) {
        return callback(self._wrapRPCError(err));
      }
      self._getBlockHashes(response.result, limit,  function(err, result) {
        if (err) {
          return callback(self._wrapRPCError(err));
        }
    ...
    self._getBlockHashes(height, limit, function (err, result) {
      if (err) {
        return callback(self._wrapRPCError(err));
      }
      callback(null, result);
...
Dash.prototype.getBlockHeader = function(blockArg, callback) {
  ...
  function queryHeader(err, blockhash) {
    ...
    self._tryAllClients(function(client, done) {
      client.getBlock(blockhash, function(err, response) {
        if (err) {
          return done(self._wrapRPCError(err));
        }
        var result = response.result;
...
Dash.prototype.getRawTransaction = function(txid, callback) {
  ...
  if (tx) {
    ...
    self._getRawTransactionData(txid, function(err, transactionData) {
      ...
      var buffer = Buffer.from(transactionData, 'hex');
...
Dash.prototype.getDetailedTransaction = function(txid, callback) {
  ...
  function addTxlockToTx(tx,result) {
    ...
    if (tx.feeSatoshis >= Dash.DEFAULT_INSTANTSEND_FEE) { // if transaction fee is >= 0.0001 check for txlock
      ...
      var id = hash.toString('binary');
      if (self.node.services.essd.zmqKnownTransactionLocks.get(id)) {
...
Dash.prototype.getMNList = function(callback){
  ...
  var prepareResponse = function (err) {
    ...
    self.client.listMasternodes(function (err, response) {
      if (err) {
        return callback(self._wrapRPCError(err));
      }
      var result = response.result;
      ...
      for (var i = 0; i < resultLength; i++) {
        var MN = {
          ...
          outidx: result[i].outidx,
          status: result[i].status,
          addr: result[i].addr,
          version: result[i].version,
          lastseen: result[i].lastseen,
          activetime: result[i].activetime,
          lastpaid: result[i].lastpaid,
          tier: result[i].tier
        };
        MNList.push(MN);
      }
...
Dash.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    var exited = false;
    this.node.stopping = true;
    var self = this
    this._stopSpawnedDash(function(err) {
      if (err) {
        return callback(err);
      }
      if (!exited) {
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('essd process did not exit'));
      }
    }, self.shutdownTimeout).unref();
    });
  } else {
    callback();
  }
