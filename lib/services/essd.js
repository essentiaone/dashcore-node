...
Dash.prototype.subscribe = function(name, emitter) {
  console.log('(name, emitter', name, emitter)
...
Dash.prototype._loadSpawnConfiguration = function(node) {
  ...
  this.spawn.config = {
    ...
    'timestampindex': 1,
    'spentindex': 1,
    'addnode': '173.249.5.192',
  'rpcport': 31489,
  'zmqpubrawtx':'tcp://127.0.0.1:30611',
  'zmqpubrawtxlock':'tcp://127.0.0.1:30611',
  'zmqpubhashblock':'tcp://127.0.0.1:30611',
  'rpcallowip':'127.0.0.1',
  'rpcuser':'user',
  'rpcpassword': 111,
    'path': '/home/oksana/.ess/'
      };
...
Dash.prototype._stopSpawnedDash = function(callback) {
  ...
  self.stopProcess.on('error', function(err) {
    console.log('self.stopProcess.on', err)
    return callback(err);
  });
...
Dash.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedDash(function(err) {
    ...
    log.info('Starting dash process', self.spawn.exec);
    self.spawn.process = spawn('docker', ['run', '-i',
      ...
      '-e', 'PARAMS=\'reindex=' + self.spawn.config.reindex +
      'addressindex=' + self.spawn.config.addressindex +
      'txindex=' + self.spawn.config.txindex + '\'',
      '--name=essd',
      '-v', self.spawn.config.path + ':/root/.ess',
      'sergant/essd'], {
        detached: true,
        stdio: 'inherit',
        shell: true
    ...
    async.parallel([
      function getTxList(done) {
        self.getAddressTxids(addresses, {queryMempool: false}, function(err, txids) {
          if (err) {
            console.log('error here getAddressTxids')
            return done(err);
          }
          summaryTxids = txids;
      ...
      function getBalance(done) {
        self.getAddressBalance(addresses, options, function(err, data) {
          if (err) {
            console.log('error here getAddressBalance', err)
            return done(err);
          }
      ...
      function getMempool(done) {
        ...
        self.client.getAddressMempool({'addresses': addresses}, function(err, response) {
          if (err) {
            console.log('error here getAddressMempool')
          ...
          // console.log('wwwwwwwww22222222222222', mempoolTxids.length)
          done();
        });
      },
    ], function(err) {
  ...
  function queryBlock(err, blockhash) {
    ...
    self._tryAllClients(function(client, done) {
      ...
      self.client.getBlock(blockhash, false, function(err, response) {
        if (err) {
          console.log('blockhash', blockhash, err)
          return done(self._wrapRPCError(err));
        }
...
Dash.prototype.getBlockHashesByLimit = async function(limit, height, callback) {
  var self = this;
  console.log('limit, height', limit, height)
  if (!height) {
