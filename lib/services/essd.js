...
Dash.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
  log.info('_rapidProtectedUpdateTip', new Date() - self.lastTip > 1000)
...
Dash.prototype._stopSpawnedDash = function(callback) {
  ...
  self.stopProcess.once('exit', function(code) {
    log.warn('KILL essd process with code:', code);
...
Dash.prototype._spawnChildProcess = function(callback) {
  ...
  ///////////////////////////////////////////////////////////////
  // comment this for starting essd from console
  // start
  //////////////////////////////////////////////////////////////
  self._stopSpawnedDash(function(err) {
    ...
    log.info('Starting dash process', self.spawn.datadir);
    ...
    self.spawn.process.on('data', function(data) {
    });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      // console.log('elf.spawn.config', self.spawn.config.zmqpubrawtx)
  ...
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('essd spawned process exited with status code: ' + code);
          error.code = code;
